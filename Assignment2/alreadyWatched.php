...
<?php
  $ID = $_SESSION["ID"];
  $query = "SELECT * FROM user_status WHERE user_number = $ID AND status = 'AW';";
  $results = mysqli_query($conn, $query);
  $num_of_AW = mysqli_num_rows($results);
...
<html lang="en">
  <head>
    ...
    <title>Already Watched</title>
  </head>
  <body>
    <header>
      <div class="header-layout">
          <div><i class="fa fa-film" aria-hidden="true"></i><p>
              My Already Watched List <?php echo "(".$num_of_AW. ")"; ?>
          </p>
          </div>
